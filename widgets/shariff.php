...
return [
    ...
    'events' => [
        'view.scripts' => function ($event, $scripts) use ($app) {
            $scripts->register('shariff-widget',
                'spqr/shariff:app/bundle/shariff-widget.js', ['~widgets']);
        },
    ],
    ...
    'render' => function ($widget) use ($app) {
        ...
        foreach ($config as $key => $conf) {
            switch ($key) {
                case 'orientation' :
                    if ($widget->get('overwrite_orientation')
                        && !empty($widget->get('orientation'))
                    ) {
                ...
                case 'flattr_category' :
                    if ($widget->get('overwrite_services')
                        && !empty($widget->get('flattr_category'))
                    ) {
                        $data[] = "data-flattr-category='"
                            .$widget->get('flattr_category')."'";
                    } elseif (!empty($conf)) {
                ...
                case 'flattr_user' :
                    if ($widget->get('overwrite_services')
                        && !empty($widget->get('flattr_user'))
                    ) {
                        $data[] = "data-flattr-user='"
                            .$widget->get('flattr_user')."'";
                    } elseif (!empty($conf)) {
                        $data[] = "data-flattr-user='$conf'";
                    }
                    break;
                case 'lang' :
                    if ($widget->get('overwrite_lang')
                        && !empty($widget->get('lang'))
                    ) {
                ...
                case 'services' :
                    ...
                    break;
                case 'theme' :
                    if ($widget->get('overwrite_theme')
                        && !empty($widget->get('theme'))
                    ) {
